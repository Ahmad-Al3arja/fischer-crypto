...
public class WithdrawalService {
    ...
    @Value("${app.business.withdrawal.fee-percentage:12.00}")
    private BigDecimal withdrawalFeePercentage;
    ...
    public TransactionDTOs.WithdrawalResponse createWithdrawal(User user, TransactionDTOs.WithdrawalRequest request) {
        ...
        // Calculate fee using configured percentage
        BigDecimal feePercentage = withdrawalFeePercentage.divide(BigDecimal.valueOf(100));
        BigDecimal fee = request.getAmount().multiply(feePercentage);
        ...
        // Deduct the full requested amount from user balance (including the 12% fee)
        ...
        // Create response - show the net amount that will be sent to user
        TransactionDTOs.WithdrawalResponse response = new TransactionDTOs.WithdrawalResponse();
        response.setId(withdrawal.getId());
        response.setAmount(withdrawal.getAmount()); // This is the net amount (after 12% fee)
        response.setFee(BigDecimal.ZERO); // Fee is 0 since we're saving net amount
        response.setNetAmount(withdrawal.getNetAmount()); // Same as amount
