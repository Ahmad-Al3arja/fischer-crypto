...
public class UserService {
    ...
    public List<UserDTOs.ReferralDetail> getRecentReferralDetails(User user) {
        ...
        for (User referredUser : directReferrals) {
            ...
            BigDecimal commission = referralEarningRepository.findByReferrerAndReferredUser(user, referredUser)
                    .stream()
        ...
        for (User grandReferredUser : secondLevelReferrals) {
            ...
            BigDecimal commission = referralEarningRepository.findByReferrerAndReferredUser(user, grandReferredUser)
                    .stream()
                    .map(ReferralEarning::getAmount)
                    .reduce(BigDecimal.ZERO, BigDecimal::add);
            detail.setCommissionEarned(commission);
