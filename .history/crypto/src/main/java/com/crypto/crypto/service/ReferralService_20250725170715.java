...
public class ReferralService {
    ...
    private void addReferralEarning(User referrer, User referredUser, BigDecimal amount, CommissionType type) {
        ...
        earning.setReferrer(referrer);  // Set the referrer field
    ...
    public UserDTOs.DetailedReferralResponse getDetailedReferrals(User user) {
        // Get direct referrals
        List<User> directReferrals = userRepository.findByReferrer(user);
        List<UserDTOs.DetailedReferralInfo> directReferralDetails = directReferrals.stream()
                .map(this::convertToDetailedReferralInfo)
                .collect(Collectors.toList());
        
        // Get second level referrals
        List<User> secondLevelReferrals = userRepository.findByGrandReferrer(user);
        List<UserDTOs.DetailedReferralInfo> secondLevelDetails = secondLevelReferrals.stream()
                .map(this::convertToDetailedReferralInfo)
                .collect(Collectors.toList());
        
        UserDTOs.DetailedReferralResponse response = new UserDTOs.DetailedReferralResponse();
        response.setDirectReferrals(directReferralDetails);
        response.setSecondLevelReferrals(secondLevelDetails);
        response.setTotalDirectCount(directReferrals.size());
        response.setTotalSecondLevelCount(secondLevelReferrals.size());
        response.setTotalReferralEarnings(user.getReferralEarnings());
        
        return response;
    }

    public UserDTOs.ReferralEarningsHistoryResponse getReferralEarningsHistory(User user) {
        List<ReferralEarning> earnings = referralEarningRepository.findByReferrerOrderByCreatedAtDesc(user);
        List<UserDTOs.ReferralEarningDetail> earningDetails = earnings.stream()
                .map(this::convertToReferralEarningDetail)
                .collect(Collectors.toList());
        ...
        return new UserDTOs.ReferralEarningsHistoryResponse(earningDetails, totalEarnings);
    }

    private UserDTOs.DetailedReferralInfo convertToDetailedReferralInfo(User referral) {
        UserDTOs.DetailedReferralInfo info = new UserDTOs.DetailedReferralInfo();
        info.setId(referral.getId());
        info.setUsername(referral.getDisplayUsername());
        info.setFullName(referral.getFullName());
        info.setPhoneNumber(referral.getPhoneNumber());
        info.setPlanName(referral.getCurrentPlan() != null ? referral.getCurrentPlan().getName() : "No Plan");
        info.setStatus(referral.getStatus().name());
        info.setTotalBalance(referral.getTotalBalance());
        info.setJoinedAt(referral.getCreatedAt());
        ...
        BigDecimal commissionEarned = referralEarningRepository.findByReferrerAndReferredUser(getCurrentUser(), referral)
                ...
                .map(ReferralEarning::getAmount)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        info.setCommissionEarned(commissionEarned);
        
        return info;
    }

    private UserDTOs.ReferralEarningDetail convertToReferralEarningDetail(ReferralEarning earning) {
        UserDTOs.ReferralEarningDetail detail = new UserDTOs.ReferralEarningDetail();
        detail.setId(earning.getId());
        detail.setReferredUsername(earning.getReferredUser().getDisplayUsername());
        detail.setReferredFullName(earning.getReferredUser().getFullName());
        detail.setAmount(earning.getAmount());
        detail.setCommissionType(earning.getCommissionType().name());
        detail.setDepositAmount(earning.getDeposit() != null ? earning.getDeposit().getAmount() : null);
        detail.setCreatedAt(earning.getCreatedAt());
        return detail;
    }

    public UserDTOs.TeamEarningsBreakdown getTeamEarningsBreakdown(User user) {
        UserDTOs.TeamEarningsBreakdown breakdown = new UserDTOs.TeamEarningsBreakdown();
        // Direct referrals
        List<User> directReferrals = userRepository.findByReferrer(user);
        List<UserDTOs.ReferralDetail> directDetails = directReferrals.stream()
                .map(this::convertToReferralDetail)
                .collect(Collectors.toList());
        breakdown.setDirectReferralDetails(directDetails);
        breakdown.setDirectReferrals(directDetails.size());
        // Grand referrals
        List<User> grandReferrals = userRepository.findByGrandReferrer(user);
        List<UserDTOs.ReferralDetail> grandDetails = grandReferrals.stream()
                .map(this::convertToReferralDetail)
                .collect(Collectors.toList());
        breakdown.setGrandReferralDetails(grandDetails);
        breakdown.setGrandReferrals(grandDetails.size());
        // Earnings
        BigDecimal totalDirect = directDetails.stream().map(UserDTOs.ReferralDetail::getCommissionEarned).reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal totalGrand = grandDetails.stream().map(UserDTOs.ReferralDetail::getCommissionEarned).reduce(BigDecimal.ZERO, BigDecimal::add);
        breakdown.setTotalDirectEarnings(totalDirect);
        breakdown.setTotalGrandEarnings(totalGrand);
        return breakdown;
    }
} 