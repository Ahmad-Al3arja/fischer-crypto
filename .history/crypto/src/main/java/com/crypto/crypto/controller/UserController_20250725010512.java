...
@GetMapping("/team-detailed-stats")
public ResponseEntity<?> getDetailedTeamStats() {
    try {
        User currentUser = userService.getCurrentUser();
        UserDTOs.TeamStatsResponse response = userService.getTeamStats(currentUser);
        return ResponseEntity.ok(response);
...
public ResponseEntity<?> getMyReferralInfo() {
    try {
        ...
        // Stats
        response.put("directReferrals", userRepository.countDirectReferrals(currentUser));
        response.put("secondLevelReferrals", userRepository.countSecondLevelReferrals(currentUser));
        response.put("totalReferralEarnings", currentUser.getReferralEarnings());
        
        // Recent referrals details (last 10)
        List<UserDTOs.ReferralDetail> recentReferrals = userService.getRecentReferralDetails(currentUser);
        response.put("recentReferrals", recentReferrals.stream().limit(10).collect(Collectors.toList()));
