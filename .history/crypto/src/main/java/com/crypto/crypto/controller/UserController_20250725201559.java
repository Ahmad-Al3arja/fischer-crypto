...
public class UserController {
    ...
    public ResponseEntity<?> getUserDailyCounter() {
        try {
            ...
            UserDTOs.DailyCounterResponse response = dailyCounterService.getUserDailyCounter(user);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        }
    ...
    public ResponseEntity<?> getReferralLink() {
        try {
            User user = userService.getCurrentUser();
            String referralLink = userService.getPlatformBaseUrl() + "/register?ref=" + user.getDisplayUsername();
            Map<String, String> response = new HashMap<>();
            response.put("referralLink", referralLink);
            response.put("referralCode", user.getDisplayUsername());
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        }
    ...
    @GetMapping("/referrals/detailed")
    public ResponseEntity<?> getDetailedReferrals() {
        try {
            User user = userService.getCurrentUser();
            UserDTOs.DetailedReferralResponse response = referralService.getDetailedReferrals(user);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        }
    ...
    @GetMapping("/referrals/earnings-history")
    public ResponseEntity<?> getReferralEarningsHistory() {
        try {
            User user = userService.getCurrentUser();
            UserDTOs.ReferralEarningsHistoryResponse response = referralService.getReferralEarningsHistory(user);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        }
    ...
    @GetMapping("/team/earnings-breakdown")
    public ResponseEntity<?> getTeamEarningsBreakdown() {
        try {
            User user = userService.getCurrentUser();
            UserDTOs.TeamEarningsBreakdown response = referralService.getTeamEarningsBreakdown(user);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        }
    ...
    @GetMapping("/daily-counter/status")
    public ResponseEntity<?> getCounterStatus() {
        try {
            User user = userService.getCurrentUser();
            UserDTOs.DailyCounterResponse response = dailyCounterService.getUserDailyCounter(user);
            return ResponseEntity.ok(response);
