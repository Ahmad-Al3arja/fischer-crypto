...
public class UserService {
    ...
    // Enhanced phone number patterns - more flexible
    ...
    public AuthDTOs.LoginResponse register(AuthDTOs.RegisterRequest request) {
        ...
        // Enhanced phone number processing - more flexible
        String normalizedPhone = normalizePhoneNumber(request.getPhoneNumber());

        // Validate phone number format
        if (!isValidPhoneNumber(normalizedPhone)) {
            throw new RuntimeException("Invalid phone number format. Please enter a valid phone number (8-15 digits)");
        }
        ...
        if (userRepository.existsByDisplayUsername(request.getUsername().toLowerCase())) {
        ...
        referralService.incrementReferralUsage(referrer);

        // Notify referrer
        notificationService.notifyReferrerNewReferral(referrer, user);
    ...
    /**
     * Enhanced phone number normalization - more flexible and user-friendly
     */
    private String normalizePhoneNumber(String phoneNumber) {
        ...
        if (cleaned.startsWith("00")) {
            ...
            // Remove leading zero for local numbers that might be international
            cleaned = cleaned.substring(1);
        }

        // Validate basic format
        if (cleaned.length() < 8 || cleaned.length() > 15) {
            throw new RuntimeException("Phone number must be between 8 and 15 digits");
        }
        ...
        // Check for leading zero (invalid for international format)
        if (cleaned.startsWith("0")) {
            throw new RuntimeException("Please enter your phone number without leading zero (e.g., 1234567890 instead of 01234567890)");
        }
    ...
    public void updateUserBalance(Long userId, BigDecimal amount, String reason) {
        ...
        System.out.println("Admin updated user " + user.getId() + " balance: " + oldBalance + " -> " + user.getTotalBalance() + ". Reason: " + reason);
    }
    ...
    public AdminDTOs.UserSearchResponse searchUsers(String query) {
        List<User> users = userRepository.findByDisplayUsernameContainingOrFullNameContainingOrPhoneNumberContaining(query, query, query);
        List<AdminDTOs.UserSummary> userSummaries = users.stream()
                .map(this::convertToUserSummary)
                .collect(Collectors.toList());
    ...
    private AdminDTOs.UserSummary convertToUserSummary(User user) {
        ...
        summary.setDirectReferrals((int) userRepository.countDirectReferrals(user));
        summary.setReferralEarnings(user.getReferralEarnings());
