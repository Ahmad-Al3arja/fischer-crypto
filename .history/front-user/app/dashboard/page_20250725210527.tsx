...
export default function DashboardPage() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      if (data.counterStatus) {
        ...
        console.log("Backend timer state:", { isActive, remainingSeconds, isCompleted })
        ...
        console.log("Updated local timer state:", { 
          isTimerActive: timerActive && !timerCompleted, 
          displayTimeSeconds: Math.max(0, remainingSeconds) 
        })
      }
    } catch (err: any) {
  ...
  console.log("Timer Debug:", {
    dashboardData: dashboardData?.counterStatus,
    displayTimeSeconds,
    isTimerActive,
    timerDisplayState,
    currentPlanName: dashboardData?.currentPlanName
  })
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <Card className="bg-card border-border">
                  <CardContent className="pt-4">
                    <div className="text-center">
                      <p className="text-sm text-muted-foreground mb-1">{t('total_balance')}</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <Card className="bg-card border-border">
                  <CardContent className="pt-4">
                    <div className="text-center">
                      <p className="text-sm text-muted-foreground mb-1">{t('daily_profit')}</p>
                ...
                <Card className="bg-card border-border">
                  <CardContent className="pt-4">
                    <div className="text-center">
                      <p className="text-sm text-muted-foreground mb-1">{t('bonus_profit')}</p>
              ...
              <Card className="mb-6 bg-card border-border">
                ...
                <CardContent>
                  <div className="flex flex-col items-center space-y-4">
                    <div className={`w-32 h-32 rounded-full border-4 ${getTimerColor(displayTimeSeconds)} flex items-center justify-center bg-card relative`}>
                      {/* Progress ring */}
                      ...
                      <div className="text-center relative z-10">
                        {timerDisplayState === "active" ? (
                          ...
                          <>
                            ...
                            <div className="text-xs text-primary">{t('ready_to_claim')}</div>
                          </>
                        ) : (
                          <>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <Link href="/deposit">
                  <Button className="w-full h-16 sm:h-20 flex flex-col items-center justify-center space-y-2 bg-card border-border hover:bg-muted">
                    <TrendingUp className="h-5 w-5 sm:h-6 sm:w-6 text-foreground" />
                    <span className="text-xs sm:text-sm font-medium text-foreground">{t('deposit')}</span>
                  </Button>
                </Link>

                <Link href="/withdraw">
                  <Button className="w-full h-16 sm:h-20 flex flex-col items-center justify-center space-y-2 bg-card border-border hover:bg-muted">
                    <Banknote className="h-5 w-5 sm:h-6 sm:w-6 text-foreground" />
                    <span className="text-xs sm:text-sm font-medium text-foreground">{t('withdraw')}</span>
                  </Button>
                </Link>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <Link href="/withdrawal-history">
                  <Button className="w-full h-16 sm:h-20 flex flex-col items-center justify-center space-y-2 bg-card border-border hover:bg-muted">
                    <History className="h-5 w-5 sm:h-6 sm:w-6 text-foreground" />
                    <span className="text-xs sm:text-sm font-medium text-foreground">{t('transaction_history')}</span>
                  </Button>
                </Link>
                ...
                <Link href="/referrals">
                  <Button className="w-full h-16 sm:h-20 flex flex-col items-center justify-center space-y-2 bg-card border-border hover:bg-muted">
                    ...
                    <span className="text-xs sm:text-sm font-medium text-foreground">{t('referrals')}</span>
                  </Button>
                </Link>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Link href="/profile">
                  <Button className="w-full h-16 sm:h-20 flex flex-col items-center justify-center space-y-2 bg-card border-border hover:bg-muted">
                    <ProfileIcon className="h-5 w-5 sm:h-6 sm:w-6 text-foreground" />
                    <span className="text-xs sm:text-sm font-medium text-foreground">{t('profile')}</span>
                  </Button>
                </Link>
                ...
                <Link href="/plans">
                  <Button className="w-full h-16 sm:h-20 flex flex-col items-center justify-center space-y-2 bg-card border-border hover:bg-muted">
                    ...
                    <span className="text-xs sm:text-sm font-medium text-foreground">{t('plans')}</span>
                  </Button>
                </Link>
              </div>
            </>
