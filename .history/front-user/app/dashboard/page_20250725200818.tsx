...
export default function DashboardPage() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      if (data.counterStatus) {
        ...
        setIsTimerActive(isActive && !isCompleted && remainingSeconds > 0)
        setDisplayTimeSeconds(Math.max(0, remainingSeconds))
        setLastSyncTime(Date.now())
        
        console.log("Updated local timer state:", { 
          isTimerActive: isActive && !isCompleted && remainingSeconds > 0, 
  ...
  const getTimerDisplayState = () => {
    ...
    if (counterStatus.isActive && displayTimeSeconds > 0) return "active"
    if (counterStatus.isCompleted || displayTimeSeconds === 0) return "completed"
